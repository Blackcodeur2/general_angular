...
export class ClientReservationService {
  ...
  getReservationsByStatus(status: 'confirmee' | 'en_attente' | 'annulee'): Observable<Reservation[]> {
    return this.http.get<Reservation[]>(`${this.API}/reservations?statut=${status}`);
  }
  ...
  getReservationDetails(reservationId: number): Observable<Reservation> {
    return this.http.get<Reservation>(`${this.API}/reservations/${reservationId}`);
  }
  ...
  createReservation(data: ReservationRequest): Observable<Reservation> {
    return this.http.post<Reservation>(`${this.API}/reservations`, data);
  }
  ...
  cancelReservation(reservationId: number): Observable<any> {
    return this.http.put(`${this.API}/reservations/${reservationId}/cancel`, {});
  }
  ...
  getReservationReceipt(reservationId: number): Observable<any> {
    return this.http.get(`${this.API}/reservations/${reservationId}/receipt`, { responseType: 'blob' });
  }
}
